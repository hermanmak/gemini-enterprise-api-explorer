...
- Add a new standalone documentation file, `docs/AUTHENTICATION.md`, that describes only the end-user steps to obtain a short-lived Google Cloud access token via an already-configured Workforce Identity Federation pool/provider federated with Microsoft Entra ID, for the purpose of calling the Gemini Enterprise API.
