...
The two-phase (plan/run) orchestration flow used exclusively for the `deep_research` managed agent on the Gemini Enterprise Assist endpoint: v1alpha envelope construction, session continuity across the plan and run turns, long-lived incremental stream handling, and per-content-kind rendering in the UI.
...
<parameter name="i">Create main spec for deep-research-assist capability